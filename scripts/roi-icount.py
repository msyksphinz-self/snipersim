...
class RoiIcount:
  ...
  def _min_callback(self):
    ...
    offsets = [v for v in offsets if v not in (0, -1)]
    return min(offsets)
  ...
  def setup(self, args):
    ...
    if start.startswith('roi+'):
      ...
      self.roirel = False
    self.init_length = int(start or 0)
    self.warmup_length = int(args.get(1, '') or 0)
    self.detailed_length = int(args.get(2, '') or 0)

    if self.detailed_length < 1:
      print('[ROI-ICOUNT] Detailed instrucion count cannot be 0', file=sys.stderr)
      sim.control.abort()
    if sim.config.get_bool('general/magic') or not sim.config.get_bool('general/roi_script'):
      print('[ROI-ICOUNT] Need --roi-script to be set, --roi must not be used', file=sys.stderr)
      sim.control.abort()
    ...
    if self.roirel:
      ...
      self.state = 'init'
      self.offset = 0
    sim.util.EveryIns(int(sim.config.get('core/hook_periodic_ins/ins_global')), self.periodic_ins, roi_only = False)
