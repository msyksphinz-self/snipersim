...
void QemuFrontend::threadStart(unsigned int threadid)
{
   ...
   try
   {
      ...
      // Note: We need to map decoder registers to QEMU plugin registers
      // For now, we allocate based on the maximum number of registers
                              ...
                              int64_t num, uint64_t args[6])
{
   try
   {
      ...
      for (int i = 0; i < 6; i++) {
         syscall_args[i] = args[i];
      }
      FrontendSyscallModel<QemuFrontend>::handleSyscall(threadid, num, syscall_args);
   }
   catch (...)
   {
      abort();
